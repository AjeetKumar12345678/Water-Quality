...
st.title("💧 AI-Powered Water Quality Assessment System (Free Tier)")
st.markdown("Assess water safety using **Fuzzy Logic** and **Google Gemini (Free AI)** for expert insights.")
...
if "GEMINI_API_KEY" in st.secrets:
    ...
    st.sidebar.success("🔑 System Ready (Gemini API Loaded)")
else:
    # Fallback text box if secrets aren't set up yet
...
if st.button("Run Water Assessment", type="primary"):
    if not api_key:
        st.error("API Key is missing. Please add your Gemini API Key to Streamlit Secrets.")
    else:
        with st.spinner("Calculating fuzzy logic index and generating Gemini AI report..."):
            ...
            try:
                ...
                # Display LLM Report
                st.markdown("### 📋 AI Expert Water Analysis Report (Gemini)")
                st.write(report)

            except Exception as e:
                st.error("⚠️ Fuzzy logic passed, but Gemini AI generation encountered an error.")
